...
class Setup extends Component {
  ...
  imageHandler = (files, property) => {
    ...
    reader.onloadend = () => {
      ...
      if (this.state.picture) pictureObject = this.state.picture;
  ...
  getThumbnails = links => {
    ...
    axios
      .get(
        `http://free.pagepeeker.com/v2/thumbs.php?size=x&url=${links[0].url}`
      )
      .then(res => {
        console.log(res);
  ...
  BackStep = () => {
    //skipping over selection since links is not ready
    let newStep = this.state.step;
    if (newStep == 6) newStep += -2;
    else newStep--;
    this.setState({ step: newStep });
  };
  InputStep = event => {
    ...
    if (step === 1 && !userInput.match(/^[a-zA-Z" "]*$/)) return;
    else {
      switch (step) {
        case 1:
        ...
        case 2: //choose picture
          ...
          break;
        case 5:
          if (!picture.picture1) picture.picture1 = {};
          if (!picture.picture2) picture.picture2 = {};
          if (!picture.picture3) picture.picture3 = {};
  ...
  QuestionStep = () => {
    ...
    switch (step) {
      ...
      case 2:
        ...
        return (
          <PickPicture
            ...
            picture={this.state.picture ? this.state.picture.profile_pic : ''}
          />
        );
      case 3:
        return (
      ...
      case 5: //skipping over selection since links is not ready
        let skip = this.state.userInfo;
        skip.profile_type = 'pictures';
        if (!picture.picture1) picture.picture1 = {};
        if (!picture.picture2) picture.picture2 = {};
        if (!picture.picture3) picture.picture3 = {};
        this.setState({ picture: picture });
        this.NextStep();
        this.setState({ userInfo: skip });
  ...
  render() {
    ...
    return (
     /* <StyleRoot>
        ...
        <div style={style} /* className="setup" */>
          <h2>{`Let's get this page up! You're at step ${this.state.step}/7`}</h2>
          {this.QuestionStep()}
          {this.state.fireRedirect ? (
            <Redirect push to={`/site/${this.state.userInfo.name}`} />
          ) : (
            ''
          )}
        </div>
...
export default /* Radium(Setup) */ Setup;
